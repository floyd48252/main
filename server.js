...
if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
    ...
    transporter = {
        sendMail: async(mailOptions) => {
            console.log('Email send simulated because EMAIL_USER/EMAIL_PASS are not configured:', mailOptions);
...
app.post('/api/auth', async(req, res) => {
    ...
    if (action === 'login') {
        ...
        if (email === adminCredentials.email && password === adminCredentials.password) {
            ...
            const adminUser = { email, type: 'admin', name: 'Administrator', token };
    ...
    if (action === 'register') {
        ...
        // Send admin notification email with new customer details
        const registrationEmail = `New customer registration received.\n\nName: ${newUser.first_name} ${newUser.last_name}\nEmail: ${newUser.email}\nPhone: ${newUser.phone}\nCompany: ${newUser.company_name}\nRegistered At: ${new Date().toISOString()}\n`;

        try {
            await transporter.sendMail({
                from: process.env.EMAIL_USER || 'no-reply@local',
                to: COMPANY_EMAILS.admin,
                cc: COMPANY_EMAILS.info,
                subject: `New Customer Registration: ${newUser.email}`,
                text: registrationEmail,
                html: `<pre>${registrationEmail}</pre>`
            });
        } catch (mailerError) {
            console.warn('Email warning: unable to send registration notification.', mailerError.message || mailerError);
        }
